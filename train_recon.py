...
if __name__ == '__main__':
    ...
    for epoch in range(1, params.epochs + 1):
        ...
        for idx, (x, y) in enumerate(train_dl):
            ...
            #         svd_vals = torch.linalg.svdvals(model.A[layer][group].cpu())
            #         # normalize by number of eigenvals and channels
            #         loss_2 += mu * 1 / (params.n_channels * params.kernel_size ** 2) * torch.sum(1 - torch.abs(svd_vals.to(device)))
            for layer in range(params.n_layers):
                for group in range(params.n_groups):
                    loss_2 += mu * torch.linalg.matrix_norm(model.A[layer][group] @ model.A_inv[layer][group] - torch.eye(params.kernel_size ** 2, device=device)).sum()
            loss = loss_1 + loss_2
